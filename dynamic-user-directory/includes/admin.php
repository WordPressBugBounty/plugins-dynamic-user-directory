				 ...
				 <tr id="meta_fld_<?php echo $inc; ?>">
					...
					<td><b>Format Meta Field <?php echo $inc; ?> As</b><br>
						<select name="<?php echo $dud_option_name;?>[dud_fld_format_<?php echo $inc; ?>]" id="dud_hyperlink_flds">
							...
							<OPTION value="49" <?php echo (!empty($dud_options['dud_fld_format_' . $inc]) && $dud_options['dud_fld_format_' . $inc] == "49") ? "SELECTED" : ""; ?>>Multiselect => Show Values and Labels (Bulleted)</OPTION>
							...
							<OPTION value="50" <?php echo (!empty($dud_options['dud_fld_format_' . $inc]) && $dud_options['dud_fld_format_' . $inc] == "50") ? "SELECTED" : ""; ?>>Multiple Checkboxes => Show Values and Labels (Bulleted)</OPTION> 
...
function dynamic_ud_load_meta_keys($meta_type) {
	...
	else
	{
		...
		$results = $wpdb->get_results("SELECT user_id FROM " . DUD_WPDB_PREFIX . "usermeta ORDER BY RAND() LIMIT 200");
		...
		if($results)
		{
			...
			foreach ($results as $result)
			{
				$meta_vals = get_user_meta( $result->user_id ); 
				
				if($meta_vals == false || empty($meta_vals)) continue;
				
				$all_meta_for_user = array_map( function( $a ){ return $a[0]; }, $meta_vals );

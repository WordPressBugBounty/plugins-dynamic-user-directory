...
{ 	
	...
	{   		
		...
		if($user_directory_sort === "last_name")
		{
			...
			$user_last_name = ltrim($user_last_name);
		}
		else
		{
			$user_id = $uid->ID; 
		...
		{		  
		   if($user_directory_meta_flds_tmp[$inc]['field']) 
		   {
			   ...
			   $user_directory_meta_flds_tmp[$inc]['value'] =  dynamic_ud_format_meta_val($user_meta_fld, $dud_options, $fld_format, $fld_label,
					$user_directory_meta_flds_tmp[$inc]['field']); 	
			}
		} 
...
function dynamic_ud_format_meta_val($user_meta_fld, $dud_options, $format, $label = null, $meta_fld_key_name = null)
{	
	$parsed_val = "";
	$numeric_idx = false;
	$key_val_array = false;
		...
		foreach ($user_meta_fld as $key => $value) 
		{
			...
			{				
				...
				else                 
				{
					...
					//Hide Hyphens & Capitalization
					if($format === "5" || $format === "39" || $format === "30" 
					   || $format === "37" || $format === "38" || $format === "40"
					   || $format === "4"  || $format === "41" || $format === "42"
					   || $format === "45" || $format === "43" | $format === "44"
					   || $format === "47" || $format === "13" || $format === "15" 
					   || $format === "48" || $format === "49" || $format === "50"
					   || $format === "35" || $format === "36" || $format === "11" 
					   || $format === "8"  || $format === "53" || $format === "54")
					{
						$value = str_replace ( "-", " ", $value);
						
						if($format === "37" || $format === "39" || $format === "41" || $format === "43" || $format === "44" || $format === "53")
							$value = strtoupper($value);	
						else if($format === "38" || $format === "40" || $format === "42" || $format === "47" || $format === "48" || $format === "54")
							$value = strtolower($value);
						else if(!$is_member_press)
							$value = ucwords($value);
					}
